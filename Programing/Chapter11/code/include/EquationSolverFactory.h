...
public:
    using CreateEquationSolverCallback = std::unique_ptr<Solver> (*)(int);
private:
...
public:
    ...
    std::unique_ptr<Solver> createEquationSolver(const std::string &ID) {
        std::string method;
        int order = 0;
        size_t pos = 0;
...
inline void RegisterAllEquationSolvers() {
    ...
    fac.registerEquationSolver("BE", &createBackwardEulerSolver);
    fac.registerEquationSolver("CN", &createCrankNicolsonSolver);
    fac.registerEquationSolver("FE", &createForwardEulerSolver);
    fac.registerEquationSolver("RK", &createOneStageRungeKuttaSolver);
    fac.registerEquationSolver("Col", &creatCollocationSolver);
}
// 注册完成，可以创建实例了