...
int main() {
    ...
    std::ifstream ifs("../test/test_relax.json");
    ...
    if(Dim == 2){ 
        ...
        try{
            ...
            Multigrid<20> mg(func, boundaryFunc, boundaryFuncx, boundaryFuncy, boundaryCondition, exact);
            // mg.generateSparseMatrix(pixelNum);
            // mg.test_relaxation(pixelNum, nu1);
            // mg.test_irestrict(pixelNum);
            mg.test_prolongate(pixelNum);
        ...
        try{
            ...
            DynamicFunction boundaryFunc(boundaryFuncName);
            DynamicFunction exact(exact_solution);
            Multigrid<10> mg(func, boundaryFunc, boundaryCondition, exact);
            mg.generateSparseMatrix(pixelNum);
            mg.test_relaxation(pixelNum, nu1);
        } catch (const std::exception& e) {
            std::cerr << "Error: " << e.what() << std::endl;
